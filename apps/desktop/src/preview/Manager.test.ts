...
describe("PreviewManager", () => {
  ...
  effectIt.effect("scrolls the nearest container at a viewport coordinate", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        const scrollEvaluation = sendCommand.mock.calls.find(
          ([method, params]) =>
            ...
            "expression" in params &&
            typeof params.expression === "string" &&
            params.expression.includes("document.elementFromPoint(160, 240)"),
        );
        expect(scrollEvaluation).toBeDefined();
        expect(scrollEvaluation?.[1]).toEqual(
          expect.objectContaining({
            expression: expect.stringMatching(
              /element = element\.parentElement[\s\S]*element\.scrollHeight > element\.clientHeight[\s\S]*document\.scrollingElement \|\| window[\s\S]*target\.scrollBy\(\{ left: 15, top: -20, behavior: "instant" \}\)/,
            ),
            awaitPromise: true,
            returnByValue: true,
          }),
