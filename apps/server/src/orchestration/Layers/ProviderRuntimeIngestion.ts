...
const make = Effect.gen(function* () {
  ...
  const appendBufferedReasoningText = (messageId: MessageId, delta: string) =>
    Cache.getOption(bufferedReasoningTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Effect.gen(function* () {
          ...
          if (nextText.length <= MAX_BUFFERED_ASSISTANT_CHARS) {
            yield* Cache.set(bufferedReasoningTextByMessageId, messageId, nextText);
            return "";
          }
          yield* Cache.invalidate(bufferedReasoningTextByMessageId, messageId);
          return nextText;
        }),
      ),
    );
  ...
  const clearAssistantMessageState = (messageId: MessageId) =>
    Effect.all([clearBufferedAssistantText(messageId), clearBufferedReasoningText(messageId)]).pipe(
      Effect.asVoid,
    );
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (reasoningDelta && reasoningDelta.length > 0) {
        ...
        const streamingMode = yield* resolveResponseStreamingMode(thread.projectId);
        if (streamingMode !== "token") {
          const spillChunk = yield* appendBufferedReasoningText(
            assistantMessageId,
            reasoningDelta,
          );
          if (spillChunk.length > 0) {
