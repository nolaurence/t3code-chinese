    ...
    <div
      ...
      aria-label={
        process.isServerRoot
          ? t("diagnostics.resource.rootProcess", { name })
          : t("diagnostics.resource.childProcess", { name })
      }
    >
      <span className="size-5 shrink-0" aria-hidden="true" />
      <span
...
function ResourceHistoryWindowSelector({
  ...
  return (
    <ToggleGroup
      aria-label={t("diagnostics.historyPeriodAria")}
      variant="segmented"
...
function DiagnosticsLastChecked({ checkedAt }: { checkedAt: DateTime.Utc | null }) {
  ...
  const time = relative.suffix ? `${relative.value} ${relative.suffix}` : relative.value;
  return (
    <span className="text-[11px] text-muted-foreground/60">
      {t("diagnostics.checked", { time })}
    </span>
  );
}
