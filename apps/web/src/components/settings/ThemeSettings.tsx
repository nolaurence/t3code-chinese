...
function ThemeLibraryCard({
  ...
  const modeName = (mode: "light" | "dark" | "system") =>
    mode === "system"
  ...
  return (
    ...
    <Tooltip>
        ...
        render={
          ...
          >
            <div className="relative">
              {variantNavigation ? (
                <div
                  aria-label={t("theme.variantsAria")}
                      ...
                      <div className="contents" key={mode}>
                        ...
                        >
                          <button
                            aria-label={
                              options.length > 1
                                ? t("theme.chooseVariantAria", {
                                    mode: modeLabel,
                                    count: options.length,
                                    label: selected.option.label,
                                  })
                                : t("theme.useVariantAria", {
                                    mode: modeLabel,
                                    label: selected.option.label,
                                  })
                            }
                        ...
                        {options.length > 1
                          ? options.map(({ option, preview }, optionIndex) => {
                              ...
                              return (
                                <ThemeVariantTooltip
                                  key={option.label}
                                  label={t("theme.useOptionForMode", {
                                    label: option.label,
                                    mode: modeLabel,
                                  })}
                                >
                                  <button
                                    aria-label={
                                      optionIsActive
                                        ? t("theme.useOptionForModeActive", {
                                            label: option.label,
                                            mode: modeLabel,
                                          })
                                        : t("theme.useOptionForMode", {
                                            label: option.label,
                                            mode: modeLabel,
                                          })
                                    }
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    aria-label={
                      variantNavigation
                        ? isActive
                          ? t("theme.useCollectionVariantActiveAria", {
                              collection: variantNavigation.collectionLabel,
                              variant: theme.label,
                            })
                          : t("theme.useCollectionVariantAria", {
                              collection: variantNavigation.collectionLabel,
                              variant: theme.label,
                            })
                        : isActive
                          ? t("theme.useThemeActiveAria", { label: theme.label })
                          : t("theme.useThemeAria", { label: theme.label })
                    }
                    aria-pressed={isActive}
                  ...
                  {onDuplicate ? (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <Button
                            aria-label={t("theme.duplicateAria", { label: theme.label })}
                            size="icon-xs"
                            variant="ghost"
                            onClick={(event) => {
                              event.stopPropagation();
                      ...
                      />
                      <TooltipPopup>{t("theme.duplicate")}</TooltipPopup>
                    </Tooltip>
                  ) : null}
                  {onEdit ? (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <Button
                            aria-label={t("theme.editAria", { label: theme.label })}
                            size="icon-xs"
                            variant="ghost"
                            onClick={(event) => {
                              event.stopPropagation();
                      ...
                      />
                      <TooltipPopup>{t("theme.editor.edit")}</TooltipPopup>
                    </Tooltip>
                  ) : null}
                  {onDownload ? (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <Button
                            aria-label={t("theme.exportAria", { label: theme.label })}
                            size="icon-xs"
                            variant="ghost"
                            onClick={(event) => {
                              event.stopPropagation();
                      ...
                      />
                      <TooltipPopup>{t("theme.export")}</TooltipPopup>
                    </Tooltip>
                  ) : null}
                  {onRemove ? (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                          <Button
                            aria-label={
                              variantNavigation
                                ? t("theme.removeCollectionAria", {
                                    collection: variantNavigation.collectionLabel,
                                  })
                                : t("theme.removeNamedAria", { label: theme.label })
                            }
                            size="icon-xs"
                            variant="ghost"
                      ...
                      />
                      <TooltipPopup>
                        {variantNavigation ? t("theme.removeThemes") : t("theme.removeTheme")}
                      </TooltipPopup>
                    </Tooltip>
                  ) : null}
                </div>
      ...
      <TooltipPopup>
        {variantNavigation
          ? t("theme.useFirstVariants")
          : cardModes.length > 1
            ? t("theme.useForBoth")
            : t("theme.useForModeOnly", { mode: modeName(cardModes[0] ?? "light") })}
      </TooltipPopup>
    </Tooltip>
  );
}
...
export function ThemeLibrary({
  ...
  const notifyThemeSaveFailure = useCallback(() => {
    toastManager.add(
      stackedThreadToast({
        type: "error",
        title: t("theme.saveSelectionFailed"),
  ...
  const renderModeTiles = () => (
    <div
      aria-label={t("theme.appearanceModeAria")}
      ...
      {(["system", "light", "dark"] as const).map((mode) => {
        ...
        return (
          <button
            aria-label={
              mode === "system"
                ? t("theme.followSystemAria")
                : t("theme.useModeAria", {
                    mode:
                      mode === "light" ? t("theme.appearance.light") : t("theme.appearance.dark"),
                  })
            }
            aria-pressed={isActive}
            className={cn(
            ...
            stackedThreadToast({
              ...
              title:
                importedThemes.length === 1
                  ? updated
                    ? t("theme.updatedNamed", { name: importedThemes[0]!.label })
                    : t("theme.addedNamed", { name: importedThemes[0]!.label })
                  : updated
                    ? t("theme.updatedCount", { count: importedThemes.length })
                    : t("theme.addedCount", { count: importedThemes.length }),
            ...
            toastManager.add(
              stackedThreadToast({
                ...
                description: t("theme.nowAppearance", {
                  appearance:
                    modes[0] === "light" ? t("theme.appearance.light") : t("theme.appearance.dark"),
                }),
              }),
            );
            return true;
      ...
      <AlertDialog open={isThemeRemovalOpen} onOpenChange={setIsThemeRemovalOpen}>
        <AlertDialogPopup>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {canRemoveCollection
                ? t("theme.removeCollectionTitle", {
                    collection: removeDialogCollectionLabel ?? "",
                  })
                : t("theme.removeNamedTitle", { label: removeDialogTheme?.label ?? "" })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {canRemoveCollection
                ? t("theme.removeCollectionDescription")
                : t("theme.removeNamedDescription")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          {canRemoveCollection ? (
            ...
            >
              {canRemoveCollection
                ? themeIdsToRemove.length > 0
                  ? t("theme.removeSelectedCount", { count: themeIdsToRemove.length })
                  : t("theme.removeSelected")
                : t("theme.removeTheme")}
            </Button>
          </AlertDialogFooter>
        </AlertDialogPopup>
