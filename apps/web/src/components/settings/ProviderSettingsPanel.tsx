...
function ProviderLastChecked({ lastCheckedAt }: { lastCheckedAt: string | null }) {
  ...
  if (lastCheckedRelative.status === "invalid") {
    return <span>{t("providers.checkedUnavailable")}</span>;
  }

  return (
    <span>
      {lastCheckedRelative.suffix
        ? t("providers.checkedAgo", {
...
function providerEnvironmentDetail(environment: EnvironmentPresentation, t: Translate): string {
  ...
  if (environment.relayManaged) return t("providers.device.t3Connect");
  if (environment.entry.target._tag === "SshConnectionTarget") return t("providers.device.ssh");
  if (isDesktopLocalConnectionTarget(environment.entry.target)) {
    return t("providers.device.local");
  }
  ...
  const deviceTabs =
    !target.scoped && !onlyPrimaryDevice && options.length > 0 ? (
      <ScrollArea hideScrollbars scrollFade className="h-11 min-w-0 flex-1 rounded-none">
        <ToggleGroup
          aria-label={t("providers.devices")}
          variant="segmented"
          className="my-2"
    ...
    <>
      {targetEnvironmentMissing ? (
        <ProviderSettingsPlaceholder
          ...
          title={t("providers.deviceUnavailable")}
          description={t("providers.deviceUnavailableDescription")}
        />
      ) : null}
      {options.length === 0 && !targetEnvironmentMissing ? (
        <ProviderSettingsPlaceholder
          ...
          description={
            isReady ? t("providers.noDevicesDescription") : t("providers.loadingDevicesDescription")
          }
        />
      ) : null}
...
export function EnvironmentProviderSettings({
  ...
  return (
    <>
            ...
            {readOnly ? (
              ...
              <>
                  ...
                  <TooltipTrigger
                    render={
                      ...
                      >
                        ...
                        <span className="sr-only">{t("providers.refreshStatus")}</span>
                ...
                <Tooltip>
                  ...
                  />
                  <TooltipPopup side="top">{t("providers.addProvider")}</TooltipPopup>
                </Tooltip>
              </>
            )}
          </div>
          ...
          <div className="min-w-0 lg:min-h-0">
            {selectedRow ? (
              ...
              <div className="p-6 text-sm text-muted-foreground">
                {targetInstanceMissing ? t("providers.instanceGone") : t("providers.noConfigured")}
              </div>
            )}
          </div>
        </div>
      ...
      <SettingsSection title={t("providers.advanced")}>
        <SettingsRow
